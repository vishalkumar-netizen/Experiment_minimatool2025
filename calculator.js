...
(() => {
  ...
  function convertMeterToFeet(meter) {
    if (isNaN(meter) || meter <= 0) return 0;
    return Math.ceil(meter * meterToFeetFactor);
  }

  function isMeterToFeetEnabled() {
    const checkbox = document.getElementById('meterToFeetConversion');
    return checkbox ? checkbox.checked : false;
  }

  // Converts input values for DA, DH, MDA, MDH from meters to feet if enabled, else returns original
  function getConvertedValue(id) {
    const el = document.getElementById(id);
    if (!el) return 0;
    const val = parseFloat(el.value) || 0;
    if (isMeterToFeetEnabled() && /da|dh|mda|mdh/i.test(id)) {
      return convertMeterToFeet(val);
    }
    // Return value as is (assumed feet or other unaffected inputs)
    return val;
  }

  // Patch the global calculate function
  const originalCalculate = window.calculate;
  window.calculate = function() {
    // Override global input getters before calling original calculate
    // Replace parseFloat(document.getElementById(...).value) with getConvertedValue('inputId')
    // For your convenience, we override the global parseFloatInput function if exists, else assign here

    // Example overrides - adjust these IDs if your inputs differ:
    window.cat1DH = getConvertedValue('cat1dh');
    window.cat1DA = getConvertedValue('cat1da');
    window.cat1MDA = getConvertedValue('cat1mda');
    window.cat1MDH = getConvertedValue('cat1mdh');

    window.lnavDH = getConvertedValue('lnavdh');
    window.lnavDA = getConvertedValue('lnavda');
    window.lnavMDA = getConvertedValue('lnavmda');
    window.lnavMDH = getConvertedValue('lnavmdh');

    window.circMDA = getConvertedValue('circmda');
    window.circMDH = getConvertedValue('circmdh');

    // THR and AD elev remain as feet without conversion
    window.thrElev = parseFloat(document.getElementById('thrElev')?.value) || 0;
    window.adElev = parseFloat(document.getElementById('adElev')?.value) || 0;

    if (originalCalculate) originalCalculate();
  };

  // Add the checkbox for Meter to Feet Conversion AD dynamically if not present
  window.addEventListener('DOMContentLoaded', () => {
    const container = document.querySelector('.top-inputs');
    if (container && !document.getElementById('meterToFeetConversion')) {
      const div = document.createElement('div');
      div.className = 'input-group';
      div.innerHTML = `
        <label>
          <input type="checkbox" id="meterToFeetConversion" />
          Meter to Feet Conversion AD
        </label>
      `;
      container.appendChild(div);
    }

    // Recalculate when checkbox toggled or inputs changed
    const checkbox = document.getElementById('meterToFeetConversion');
    if (checkbox) {
      checkbox.addEventListener('change', () => window.calculate());
    }
    document.querySelectorAll('input[type="number"]').forEach(input => {
      input.addEventListener('input', () => window.calculate());
    });
  });
})();
