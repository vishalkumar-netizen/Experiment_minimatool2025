...
(function(){
    ...
    function calculateNonCDFA(proc, cat, thrElev, lightType) {
        ...
        let mdaCalc = roundUpTo10(thrElev + mdh);
        let finalMDA = Math.max(mda, mdaCalc);

        return {
            main: `MDA: ${finalMDA}(${mdh}), RVR: ${rvrFinal}m`,
            ...
            [...NONPRECISION_PROC_250, ...NONPRECISION_PROC_300, ...NONPRECISION_PROC_350].forEach(proc=>{
                ...
                CATS.forEach(cat=>{
                    ...
                    if(!resultEl) {
                        ...
                        resultEl.style.color = "#be2222";
                        el.parentNode.appendChild(resultEl);
                    }
                    resultEl.innerText = result.noncdfa;
                });
            });
        } else {
